def generate_siteswaps(period_length: int, num_of_objects: int,
                       juggler_A_pattern: [int], min_throw: int = 2, max_throw: int = 14):
    ...
    assert [min_throw < juggler_A_pattern[i] < max_throw
            for i in range(len(juggler_A_pattern))] == [True for i in range(len(juggler_A_pattern))]
...
def fill_pattern(pattern: [int], index: int, valid_patterns, num_of_obj: int, min_throw: int = 2, max_throw: int = 14):  # recursive
    temp_pattern = pattern[:]
    for throw in range(min_throw, max_throw + 1):
        temp_pattern[index] = throw

        if None in temp_pattern:
            fill_pattern(temp_pattern, index + 2, valid_patterns, num_of_obj, min_throw, max_throw)
        else:
